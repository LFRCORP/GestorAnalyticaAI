                    ...
                    <ul className="space-y-2">
                        {[...analysisHistory].reverse().map(record => (
                            <li key={record.id}
                                ...
                                <p className="font-semibold">Análisis del {new Date(record.date).toLocaleString()}</p>
...
const ProjectView: React.FC<ProjectViewProps> = ({ project, onUpdateProject, onBack }) => {
    ...
    const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const newDocuments: Document[] = [...project.documents];
            ...
            for (let i = 0; i < fileList.length; i++) {
                ...
                if (file.type !== 'application/pdf') {
                    ...
                    failedFiles.push(`${file.name} (No es PDF)`);
                    continue;
                }
                try {
                    ...
                    let storageUrl = '';
                    if (user) {
                        storageUrl = await uploadFile(user.uid, project.id, file);
                    }

                    // Dynamic Quota: Distribute a total of ~400 images across all documents
                    const totalFiles = fileList.length;
                    const isTechnical = file.name.toUpperCase().includes('MEMORIA') || file.name.toUpperCase().includes('CALCULO');
                    let docQuota = Math.max(20, Math.floor(400 / totalFiles));
                    ...
                    // Upload page images to Storage to avoid Firestore 1MB limit
                    ...
                    if (user) {
                        for (let i = 0; i < imageParts.length; i++) {
                            ...
                            processedPages.push({
                                ...part,
                                storageUrl: pageUrl,
                                // We keep a very small preview or just the metadata if needed, 
                                // but for now we'll keep the base64 for immediate analysis 
                                // and the storageUrl for persistence.
                                // CRITICAL: To save space in Firestore, we clear the huge data string 
                                // if we have the storageUrl.
                            });
                        }
                    } else {
            ...
            onUpdateProject({ documents: newDocuments });
    ...
    }, [project.documents, onUpdateProject]);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const { analysis, sources, normativaReport } = await analyzeDocuments(project.documents, { city, state }, historyContext);
    ...
    const renderFileUpload = () => (
        <div className="bg-slate-800 p-4 rounded-lg">
            <h3 className="font-semibold text-lg mb-2">1. Cargar Documentos</h3>
            <div className="mb-4">
                <label htmlFor="file-upload" className="w-full inline-flex justify-center items-center px-4 py-2 bg-cyan-600 text-white rounded-md cursor-pointer hover:bg-cyan-700 disabled:bg-slate-500 transition-colors">
                    <Upload className="h-5 w-5 mr-2" />
                    <span>{isUploading ? 'Procesando...' : 'Seleccionar PDFs'}</span>
                </label>
                <input id="file-upload" type="file" multiple accept=".pdf" className="hidden" onChange={handleFileUpload} disabled={isUploading} />
            </div>
            {uploadProgress && (
                <div className="mb-4 p-3 bg-slate-900/50 rounded-md border border-cyan-500/30">
                    <div className="flex justify-between text-xs text-cyan-400 mb-1">
                        <span>Procesando archivo {uploadProgress.current} de {uploadProgress.total}</span>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 truncate italic">Actual: {uploadProgress.fileName}</p>
                </div>
            )}
            <div className="space-y-2">
                {project.documents.map(doc => (
                    <div key={doc.id} className="flex items-center justify-between bg-slate-700 p-2 rounded-md">
    ...
    const renderLocation = () => {
        ...
        return (
            <div className="bg-slate-800 p-4 rounded-lg">
                <h3 className="font-semibold text-lg mb-2">2. Ubicación del Proyecto</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="relative group">
                        <select 
                            ...
                            className="w-full bg-slate-700 text-white rounded-md pl-3 pr-10 py-2 text-sm focus:ring-2 focus:ring-cyan-500 appearance-none cursor-pointer"
                        >
                    ...
                    <div className="relative group">
                        <select 
                            ...
                            className="w-full bg-slate-700 text-white rounded-md pl-3 pr-10 py-2 text-sm focus:ring-2 focus:ring-cyan-500 appearance-none cursor-pointer disabled:opacity-50"
                        >
    ...
    const renderAnalysisControls = () => (
        <div className="bg-slate-800 p-4 rounded-lg flex flex-col sm:flex-row gap-4 justify-between items-center z-10">
             <button onClick={handleAnalyze} disabled={isLoading || isUploading || project.documents.length === 0} className="w-full sm:w-auto flex justify-center items-center bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-md disabled:bg-slate-500 transition-colors">
                                    ...
                                    .map((doc, i) => (
                                        <div key={i} className="flex items-center justify-between p-2 bg-slate-900/50 rounded-md border border-slate-700/50">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {doc.estado === 'presente' && <span className="text-[10px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full border border-green-500/30">Presente</span>}
                                                {doc.estado === 'faltante' && <span className="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full border border-red-500/30">Faltante</span>}
                                                {doc.estado === 'parcial' && <span className="text-[10px] bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full border border-yellow-500/30">Parcial</span>}
                                            </div>
                                        </div>
                                    ))}
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {items.map((item, i) => (
                                    <div key={i} className={`p-4 rounded-xl border ${
                                        item.estado === 'cumple' ? 'bg-green-500/5 border-green-500/20' : 
                                        item.estado === 'no_cumple' ? 'bg-red-500/5 border-red-500/20' : 
                                        'bg-slate-700/30 border-slate-600/50'
                                    }`}>
                                        <div className="flex items-start justify-between mb-2 gap-4">
                                            <span className="font-semibold text-sm text-slate-200">{item.subcategoria}</span>
                                            <div className="flex-shrink-0">
                                                {item.estado === 'cumple' && <span className="text-xs bg-green-500/20 text-green-400 px-2 py-1 rounded-full border border-green-500/30 flex items-center gap-1"><CheckCircle2 className="h-3 w-3"/> Cumple</span>}
                                                {item.estado === 'no_cumple' && <span className="text-xs bg-red-500/20 text-red-400 px-2 py-1 rounded-full border border-red-500/30 flex items-center gap-1"><AlertTriangle className="h-3 w-3"/> No Cumple</span>}
                                                {item.estado === 'no_aplica' && <span className="text-xs bg-slate-600/50 text-slate-300 px-2 py-1 rounded-full border border-slate-500/50">N/A</span>}
                                            </div>
                                        </div>
                                        <p className="text-xs text-slate-400 leading-relaxed">{item.detalle}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                                    ...
                                    <tbody className="divide-y divide-slate-700/50">
                                        {items.map((p, i) => (
                                            <tr key={i} className="hover:bg-slate-700/20 transition-colors">
                                                ...
                                                <td className="px-4 py-2">
                                                    {p.estado === 'presente' && <span className="text-[9px] bg-green-500/10 text-green-400 px-1.5 py-0.5 rounded border border-green-500/20">OK</span>}
                                                    {p.estado === 'faltante' && <span className="text-[9px] bg-red-500/10 text-red-400 px-1.5 py-0.5 rounded border border-red-500/20">FALTA</span>}
                                                    {p.estado === 'parcial' && <span className="text-[9px] bg-yellow-500/10 text-yellow-400 px-1.5 py-0.5 rounded border border-yellow-500/20">PARCIAL</span>}
                                                </td>
                                                <td className="px-4 py-2 text-center">
                                                    {p.tieneFirmas ? (
                                                        <span className="text-green-500 inline-block"><CheckCircle2 className="h-3 w-3" /></span>
                                                    ) : (
                                                        <span className="text-red-500 inline-block"><X className="h-3 w-3" /></span>
                                                    )}
                                                </td>
                                                <td className="px-4 py-2 text-[10px] text-slate-400 italic">
                                                    {p.comentarioTecnico}
                                                </td>
                                            </tr>
                                        ))}
                ...
                <div className="space-y-3">
                    {activeAnalysis.verificacionCalculos.map((calc, i) => (
                        <div key={i} className={`p-3 rounded-lg border ${
                            calc.estado === 'correcto' ? 'bg-green-500/5 border-green-500/20' : 
                            calc.estado === 'error' ? 'bg-red-500/5 border-red-500/20' : 
                            ...
                            <p className="text-xs text-slate-400 ml-4">{calc.detalles}</p>
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        <div key={i} className={`p-4 rounded-lg border ${
                            ...
                            <div className="flex items-start justify-between gap-4 mb-3">
                                ...
                                <div className="shrink-0 text-right">
                                    {item.estado === 'correcto' && <span className="text-green-400 text-sm font-medium flex items-center"><CheckCircle2 className="w-4 h-4 mr-1"/> Correcto</span>}
                                    {item.estado === 'error_critico' && <span className="text-red-400 text-sm font-bold flex items-center"><AlertTriangle className="w-4 h-4 mr-1"/> Error Crítico</span>}
                                    {item.estado === 'observacion_menor' && <span className="text-yellow-400 text-sm font-medium flex items-center"><AlertTriangle className="w-4 h-4 mr-1"/> Observación</span>}
                                    {item.estado === 'no_evaluable' && <span className="text-slate-500 text-sm font-medium">No Evaluable</span>}
                                </div>
                            </div>
                            
                            <div className="space-y-3 mt-4 text-sm">
                                <div>
                                    <span className="text-slate-400 text-xs font-semibold uppercase block mb-1">Hallazgo Técnico:</span>
                                    <p className="text-slate-300 bg-slate-900/50 p-3 rounded-md border border-slate-700/50">{item.hallazgo}</p>
                                </div>
                                
                                {item.riesgoOperativo && item.estado !== 'correcto' && item.estado !== 'no_evaluable' && (
                                    <div>
                                        <span className="text-red-400/80 text-xs font-semibold uppercase block mb-1">Riesgo Operativo / Life-Safety:</span>
                                        <p className="text-red-300 bg-red-900/20 p-3 rounded-md border border-red-500/20">{item.riesgoOperativo}</p>
                                    </div>
                                )}
                                
                                {item.recomendacion && item.estado !== 'correcto' && (
                                    <div>
                                        <span className="text-cyan-400/80 text-xs font-semibold uppercase block mb-1">Recomendación del Perito:</span>
                                        <p className="text-cyan-300 bg-cyan-900/20 p-3 rounded-md border border-cyan-500/20">{item.recomendacion}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                    ...
                                    <div key={puntoIndex} className={`p-4 rounded-lg border ${
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-3">
                                            ...
                                            <div className="shrink-0">
                                                {punto.estado === 'cumple' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/30"><CheckCircle2 className="w-3 h-3 mr-1"/> Cumple</span>}
                                                {punto.estado === 'no_cumple' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-500/20 text-red-400 border border-red-500/30"><AlertTriangle className="w-3 h-3 mr-1"/> No Cumple</span>}
                                                {punto.estado === 'parcial' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400 border border-yellow-500/30"><AlertTriangle className="w-3 h-3 mr-1"/> Parcial</span>}
                                                {punto.estado === 'no_aplica' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-500/20 text-slate-400 border border-slate-500/30">N/A</span>}
                                            </div>
                                        </div>
                                        
                                        <div className="space-y-2 text-sm">
                                            <div className="bg-slate-900/50 p-3 rounded-md">
                                                <span className="text-slate-400 block text-xs mb-1 uppercase tracking-wider font-semibold">Hallazgo en Documentos:</span>
                                                <p className="text-slate-300">{punto.hallazgo}</p>
                                            </div>
                                            
                                            {(punto.estado === 'no_cumple' || punto.estado === 'parcial') && (
                                                <div className="bg-amber-900/20 p-3 rounded-md border border-amber-500/20">
                                                    <span className="text-amber-500 block text-xs mb-1 uppercase tracking-wider font-semibold">Acción Requerida ({punto.recomendacionArea}):</span>
                                                    <p className="text-amber-200/80">{punto.recomendacionArea}</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
    ...
    const renderAnalysis = () => {
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-slate-800 rounded-lg border border-slate-700">
                    ...
                    <div className="mt-6 w-full max-w-md">
                        ...
                        <p className="mt-4 text-xs text-slate-500 text-center leading-relaxed">
                            Estamos procesando {project.documents.length} documentos. <br />
        ...
        const completeComparativeData = project.documents.map(doc => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#0b1120]">
            {/* Header Section */}
            <div className="p-4 lg:p-6 border-b border-slate-800 bg-[#0b1120]/50 backdrop-blur-md sticky top-0 z-20">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="relative group flex items-center bg-slate-900/80 border border-slate-700 rounded-lg px-2 py-1 pr-1 hover:border-cyan-500 transition-all">
                                    <select 
                                        value={project.visibility || 'private'}
                                        onChange={(e) => onUpdateProject({ visibility: e.target.value as ProjectVisibility })}
                                        className="appearance-none bg-transparent text-[10px] font-bold uppercase tracking-widest text-slate-400 border-none outline-none cursor-pointer pr-4"
                                    >
